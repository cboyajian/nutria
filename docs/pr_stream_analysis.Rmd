...
plot_k <- ggplot(data = stream_data, aes(x = sample_date, y = k / 2)) +
  ...
  scale_x_date(limits = c(as.Date("1989-01-01"), as.Date("1994-07-01")), date_breaks = "1 year") +
  labs(x = "Year", 
       y = "K mg l\u207b\u00b9",
       linetype = "")  +
  theme_classic()+
...
plot_no3 <- ggplot(data = stream_data, aes(x = sample_date, y = no3 / 3)) +
  ...
  geom_vline(xintercept = as.Date("1989-10-01"), linetype = "dashed") +
  scale_x_date(limits = c(as.Date("1989-01-01"), as.Date("1994-07-01")), date_breaks = "1 year", date_labels = "%Y") +
